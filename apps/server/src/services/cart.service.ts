...
export async function addToCart(data: AddToCart): Promise<Cart> {
  ...
  if (!product) throw new ApiError(NOT_FOUND, NOT_FOUND_PHRASE, `Product with id ${data.productId} not found`);
  ...
  if (!updatedCart) {
    throw new ApiError(INTERNAL_SERVER_ERROR, INTERNAL_SERVER_ERROR_PHRASE, "Failed to retrieve updated cart");
  }
