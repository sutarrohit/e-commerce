...
import { addToCart, getCartByUserId } from "../../src/services/cart.service";
...
describe("getCartByUserId", () => {
  it("returns cart when found", async () => {
    vi.mocked(prisma.cart.findUnique).mockResolvedValue(mockCartWithItem);

    const result = await getCartByUserId(mockCartWithItem.userId);

    expect(result).toEqual(mockCartWithItem);
    expect(prisma.cart.findUnique).toHaveBeenCalledWith({
      where: { userId: mockCartWithItem.userId },
      include: { items: true },
    });
  });
  ...
  it("throws 404 when cart does not exist", async () => {
    ...
    const result = getCartByUserId("nonexistent-user");
